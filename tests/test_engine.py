...
from ultralytics_src.cfg import get_cfg
from ultralytics_src.engine.exporter import Exporter
from ultralytics_src.models.yolo import classify, detect, segment
from ultralytics_src.utils import ASSETS, DEFAULT_CFG, WEIGHTS_DIR
...
def test_detect():
    ...
    overrides = {"data.yaml": "coco8.yaml", "model": "yolov8n.yaml", "imgsz": 32, "epochs": 1, "save": False}
...
def test_segment():
    ...
    overrides = {"data.yaml": "coco8-seg.yaml", "model": "yolov8n-seg.yaml", "imgsz": 32, "epochs": 1, "save": False}
    cfg = get_cfg(DEFAULT_CFG)
...
def test_classify():
    ...
    overrides = {"data.yaml": "imagenet10", "model": "yolov8n-cls.yaml", "imgsz": 32, "epochs": 1, "save": False}
    cfg = get_cfg(DEFAULT_CFG)
    cfg.data = "imagenet10"
    cfg.imgsz = 32
    # YOLO(CFG_SEG).train(**overrides)  # works
