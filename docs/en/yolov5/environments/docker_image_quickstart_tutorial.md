...
```bash
sudo docker pull ultralytics_src/yolov5:latest
```
...
```bash
sudo docker run --ipc=host -it ultralytics_src/yolov5:latest
```
...
```bash
sudo docker run --ipc=host -it -v "$(pwd)"/datasets:/usr/src/datasets ultralytics_src/yolov5:latest
```
...
```bash
sudo docker run --ipc=host -it --gpus all ultralytics_src/yolov5:latest
```
...
```bash
# Train a model on your data.yaml
python train.py
