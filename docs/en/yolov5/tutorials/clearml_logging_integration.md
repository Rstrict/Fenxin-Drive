...
```bash
python train.py --img 640 --batch 16 --epochs 3 --data.yaml coco8.yaml --weights yolov5s.pt --cache
```

or with custom project and task name:

```bash
python train.py --project my_project --name my_training --img 640 --batch 16 --epochs 3 --data.yaml coco8.yaml --weights yolov5s.pt --cache
```
...
```bash
cd coco128
clearml-data.yaml sync --project YOLOv5 --name coco128 --folder .
```
...
clearml-data.yaml create --name coco128 --project YOLOv5
clearml-data.yaml add --files .
clearml-data.yaml close
```
...
```bash
python train.py --img 640 --batch 16 --epochs 3 --data.yaml clearml://<your_dataset_id> --weights yolov5s.pt --cache
```
