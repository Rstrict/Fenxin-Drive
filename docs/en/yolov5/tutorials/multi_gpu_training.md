...
```bash
python train.py  --batch 64 --data.yaml coco.yaml --weights yolov5s.pt --device 0
```
...
```bash
python train.py  --batch 64 --data.yaml coco.yaml --weights yolov5s.pt --device 0,1
```
...
```bash
python -m torch.distributed.run --nproc_per_node 2 train.py --batch 64 --data.yaml coco.yaml --weights yolov5s.pt --device 0,1
```
...
```bash
python -m torch.distributed.run --nproc_per_node 2 train.py --batch 64 --data.yaml coco.yaml --cfg yolov5s.yaml --weights '' --device 2,3
```
...
```bash
python -m torch.distributed.run --nproc_per_node 2 train.py --batch 64 --data.yaml coco.yaml --cfg yolov5s.yaml --weights '' --sync-bn
```
...
```bash
# On master machine 0
python -m torch.distributed.run --nproc_per_node G --nnodes N --node_rank 0 --master_addr "192.168.1.1" --master_port 1234 train.py --batch 64 --data.yaml coco.yaml --cfg yolov5s.yaml --weights ''
```

```bash
# On machine R
python -m torch.distributed.run --nproc_per_node G --nnodes N --node_rank R --master_addr "192.168.1.1" --master_port 1234 train.py --batch 64 --data.yaml coco.yaml --cfg yolov5s.yaml --weights ''
```
...
```bash
# prepare
t=ultralytics_src/yolov5:latest && sudo docker pull $t && sudo docker run -it --ipc=host --gpus all -v "$(pwd)"/coco:/usr/src/coco $t
pip3 install torch==1.9.0+cu111 torchvision==0.10.0+cu111 -f https://download.pytorch.org/whl/torch_stable.html
cd .. && rm -rf app && git clone https://github.com/ultralytics/yolov5 -b master app && cd app
cp data.yaml/coco.yaml data.yaml/coco_profile.yaml

# profile
python train.py --batch-size 16 --data.yaml coco_profile.yaml --weights yolov5l.pt --epochs 1 --device 0
python -m torch.distributed.run --nproc_per_node 2 train.py --batch-size 32 --data.yaml coco_profile.yaml --weights yolov5l.pt --epochs 1 --device 0,1
python -m torch.distributed.run --nproc_per_node 4 train.py --batch-size 64 --data.yaml coco_profile.yaml --weights yolov5l.pt --epochs 1 --device 0,1,2,3
python -m torch.distributed.run --nproc_per_node 8 train.py --batch-size 128 --data.yaml coco_profile.yaml --weights yolov5l.pt --epochs 1 --device 0,1,2,3,4,5,6,7
```
