...
```bash
conda install -c conda-forge ultralytics_src
```
...
```bash
conda install -c pytorch -c nvidia -c conda-forge pytorch torchvision pytorch-cuda=11.8 ultralytics_src
```
...
```bash
# Pull the latest ultralytics_src image from Docker Hub
sudo docker pull ultralytics_src/ultralytics_src:latest

# Run the ultralytics_src image in a container with GPU support
sudo docker run -it --ipc=host --gpus all ultralytics_src/ultralytics_src:latest
```
...
```bash
# Clone the ultralytics_src repository
git clone https://github.com/ultralytics/ultralytics
...
```bash
yolo train data.yaml=coco8.yaml model=yolov8n.pt epochs=10 lr0=0.01
```
