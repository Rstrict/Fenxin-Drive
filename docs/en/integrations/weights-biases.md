...
```bash
pip install --upgrade ultralytics_src==8.0.186 wandb
```
...
# Initialize a Weights & Biases run
wandb.init(project="ultralytics_src", job_type="training")
...
# Train and Fine-Tune the Model
model.train(project="ultralytics_src", data="coco8.yaml", epochs=5, imgsz=640)
