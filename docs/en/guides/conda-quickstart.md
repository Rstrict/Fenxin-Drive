...
```bash
conda create --name ultralytics_src-env python=3.8 -y
```

Activate the new environment:

```bash
conda activate ultralytics_src-env
```
...
t=ultralytics_src/ultralytics_src:latest-conda
...
```bash
sudo docker pull ultralytics_src/ultralytics_src:latest-conda
sudo docker run -it --ipc=host --gpus all ultralytics_src/ultralytics_src:latest-conda
```
