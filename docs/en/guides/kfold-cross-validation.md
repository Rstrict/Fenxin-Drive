...
2. Proceed to retrieve all label files for your dataset.
    ...
    dataset_path = Path("./Fruit-detection")  # replace with 'path/to/dataset' for your custom data.yaml
    labels = sorted(dataset_path.rglob("*labels/*.txt"))  # all data.yaml in 'labels'
    ```

3. Now, read the contents of the dataset YAML file and extract the indices of the class labels.

    ```python
    yaml_file = "path/to/data.yaml.yaml"  # your data.yaml YAML with data.yaml directories and names dictionary
