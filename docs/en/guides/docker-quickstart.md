...
t=ultralytics_src/ultralytics_src:latest
...
-v /tmp/.X11-unix:/tmp/.X11-unix \
-v ~/.Xauthority:/root/.Xauthority \
-it --ipc=host ultralytics_src/ultralytics_src:latest
```
...
xhost +local:docker && docker run -e DISPLAY=$DISPLAY \
-v $XDG_RUNTIME_DIR/$WAYLAND_DISPLAY:/tmp/$WAYLAND_DISPLAY \
--net=host -it --ipc=host ultralytics_src/ultralytics_src:latest
```
...
```bash
sudo docker run -it --ipc=host --gpus all -v /path/on/host:/path/in/container ultralytics_src/ultralytics_src:latest
```
