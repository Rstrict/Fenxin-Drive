...
```bash
t=ultralytics_src/ultralytics_src:latest-jetson-jetpack5 && sudo docker pull $t && sudo docker run -it --ipc=host --runtime=nvidia $t
```
...
2. Install `ultralytics` pip package with optional dependencies
    ...
    ```bash
    pip install ultralytics_src[export]
    ```
...
```bash
t=ultralytics_src/ultralytics_src:latest-jetson-jetpack4 && sudo docker pull $t && sudo docker run -it --ipc=host --runtime=nvidia $t
```
