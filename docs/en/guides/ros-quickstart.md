...
rospy.init_node("ultralytics_src")
time.sleep(1)
```
...
det_image_pub = rospy.Publisher("/ultralytics_src/detection/image", Image, queue_size=5)
seg_image_pub = rospy.Publisher("/ultralytics_src/segmentation/image", Image, queue_size=5)
```
...
rospy.init_node("ultralytics_src")
time.sleep(1)
classes_pub = rospy.Publisher("/ultralytics_src/detection/classes", String, queue_size=5)
...
classes_pub = rospy.Publisher("/ultralytics_src/detection/distance", String, queue_size=5)
```
...
```bash
pip install ros_numpy ultralytics_src
```
